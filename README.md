...
This section uses *Udacity workspace* and the provided software to integrate the ML pipline into a simulated clinical environment for hippocampus evaluation in MRI images. [inference_dcm.py](Section3/inference_dcm.py) script is prepared to deploy the saved model on the MRI image stack, label the hippocampus, compute its volume and present the results in the image viewer as a DICOM file. 
