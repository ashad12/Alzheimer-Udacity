...
def get_predicted_volumes(pred):
    ...
    for slc in range(pred.shape[0]):
        volume_ant += len(np.argwhere(pred[slc,:,:]==1))
        volume_post += len(np.argwhere(pred[slc,:,:]==2))
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    slice_nums = [orig_vol.shape[0]//3, orig_vol.shape[2]//2, orig_vol.shape[2]*3//4] # is there a better choice?
    ...
    pimg.paste(pil_i, box=(50, 400))
    
    slice = pred_vol[slice_nums[0],:,:]
    nd_img = np.flip((slice/np.max(slice))*0xff).T.astype(np.uint8)
    
    pil_i = Image.fromarray(nd_img, mode="L").convert("RGBA").resize((512,512))
    
    pimg.paste(pil_i, box=(550, 400))
...
#     for series_for_inference in dicoms:
#         print(series_for_inference.file_meta)
...
if __name__ == "__main__":
    ...
    for i, subdir in enumerate(study_dir):
        if 'HCropVolume' in subdir.split("/")[-1]:
            study_dir = study_dir[i]
            break
    ...
    inference_agent = UNetInferenceAgent(
        device="cpu",
        parameter_file_path="inference/model.pth")
