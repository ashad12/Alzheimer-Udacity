...
class UNetExperiment:
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            # ANSWER: first dimension is the bacth-size, the second is the axial plane used for training and labeling
            # the third and forth dimensions are the extended coronal and sagittal plane i.e. patch size
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.val_loader):
                ...
                data_val = batch["image"].float().to(self.device)
                target_val = batch["seg"].to(self.device)

                pred_val = self.model(data_val)

                pred_val_softmax = F.softmax(pred_val, dim=1)
                loss_val = self.loss_function(pred_val_softmax, target_val[:, 0, :, :])

                print(f"Batch {i}. Data shape {data_val.shape} Loss {loss_val}")
    ...
    def run_test(self):
        ...
        print("Testing...")
        self.model.eval()
        self.load_model_parameters(os.path.join(self.out_dir, "model.pth"))
#         self.load_model_parameters(os.path.join('../out', '2020-06-12_0501_Basic_unet', "model.pth"))
        ...
        for i, x in enumerate(self.test_data):
            ...
            print(f"{x['filename']} Dice {dc:.4f} / Jacc. {jc:.4f}. {100*(i+1)/len(self.test_data):.2f}% complete")

        out_dict["overall"] = {
            "mean_dice": np.mean(dc_list),
            "mean_jaccard": np.mean(jc_list)}
        print(f"Overall average score:  Dice {out_dict['overall']['mean_dice']:.4f} / Jaccard {out_dict['overall']['mean_jaccard']:.4f}")
        print("\nTesting complete.")
        return out_dict
